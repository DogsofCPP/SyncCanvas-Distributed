...
> 需求来源：课堂验收/演示需要。以下任务按“低冲突”原则拆分：
> - A/B 仅改 `server/`
> - C/D 仅改 `public/`（压测相关在 `locust/`）
> - 公共协议变更必须先在 `docs/PROTOCOL.md` 落地

### 6.1 必须补齐的功能清单

1. **笔画选中后可移动/删除（仅允许操作自己画的笔画）**
   - 当前仅支持橡皮擦“擦除效果”，需要支持对“笔画对象”进行选择、移动、删除
2. **右侧在线用户列表显示正确用户名（username）且在线人数实时更新**
   - 统一以注册登录的 `username` 作为对外展示身份（username 唯一）
   - 在线人数不允许由前端猜测，必须由服务端统计并广播
4. **压测文件实现（Locust）**
   - 支持登录获取 token、建立 WS 连接、持续发送 stroke、统计延迟与成功率

### 6.2 分工（A/B/C/D）— 避免冲突版

#### B：服务端笔画“删除/移动”协议与校验（server/）

**目标**：给“选中移动/删除”提供后端协议支持，并保证只能操作自己的笔画。

- [ ] 设计并实现 WS 消息：
  - `stroke_delete`：按 `stroke_id` 删除
  - `stroke_move`：按 `stroke_id` 更新（推荐携带 `delta` 或 `new_points`）
- [ ] 权限校验：只能删除/移动自己创建的笔画（按 `user_id`/`username` 对齐）
- [ ] 持久化与广播：
  - 删除：Mongo 删除对应 stroke；广播给画布内所有人
  - 移动：Mongo 更新 stroke points（或写入 move 事件并在重放时应用）；广播给画布内所有人
- [ ] 与 A 对齐：`docs/PROTOCOL.md` 更新字段与示例 JSON

**涉及文件**：`server/index.js`、`server/mongo-client.js`（若需新增更新接口）

**验收**：A 用户移动/删除自己的某条 stroke 后，B 用户实时看到变化；刷新重放后仍保持一致。

#### C：前端渲染与交互（public/）— 选中/移动/删除 + 高亮用户笔画

**目标**：实现 UI 侧“选中笔画（仅自己）→ 移动/删除”，并支持点击右侧用户后高亮其笔画。

- [ ] 新增“选择/移动模式”（建议工具栏增加按钮或快捷键）：
  - 仅在选择模式下进行命中测试，避免干扰绘画
- [ ] **笔画命中测试（Hit Test）**：
  - 优先 bbox 粗筛，再点到折线距离阈值精筛
  - 仅允许选中 `operation.user_id === currentUserId` 的笔画
- [ ] **移动**：拖拽选中笔画，更新 points；拖拽结束发送 `stroke_move`
- [ ] **删除**：Delete 键/按钮删除选中笔画，发送 `stroke_delete`
- [ ] **高亮用户笔画**：
  - 点击右侧用户名后，设置 `highlightUserId`
  - 重绘时对命中用户笔画强调（描边/不透明度），其他弱化

**涉及文件**：`public/draw.js`、（可选）`public/index.html`、`public/style.css`

**验收**：
- 自己画的任意一条笔画可被选中并移动/删除；他人笔画不可被选中
- 点击右侧用户后，该用户的笔画在画布中明显高亮

#### D：压测（locust/）

**目标**：提供可运行的 Locust 压测脚本，覆盖“登录→WS→持续画笔→统计延迟”。

- [ ] `locust/locustfile.py`：
  - HTTP 登录获取 token
  - 连接 `ws://localhost:3000/ws?canvas_id=...&token=...`
  - 持续发送 stroke（模拟 50ms~200ms）
  - 统计：连接成功率、发送成功率、端到端延迟（可在 payload 带 timestamp）
- [ ] 压测场景：
  - 50/100/200 并发用户
  - 同画布高频作画

**验收**：能在本机跑通并产出基础数据（RPS/失败率/延迟）。

### 6.3 协作约定（避免冲突）

- 公共协议字段与新增消息类型：先改 `docs/PROTOCOL.md` 再写代码
- A/B 不改 `public/`；C/D 不改 `server/`
- 右侧用户点击高亮的前端接口建议采用事件：
  - `window.dispatchEvent(new CustomEvent('synccanvas:highlight-user', { detail: { userId } }))`
